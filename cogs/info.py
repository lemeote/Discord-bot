    ...
    async def info(self, interaction: Interaction, coin_symbol: str):
        ...
        if coin_id is None:
            async with ClientSession() as session:
                ...
                for coin in data:
                    if (
                        coin["symbol"] == coin_symbol.lower()
                        and "peg" not in coin["id"].lower()
                    ):
                        coin_id = coin["id"]
        ...
        async with ClientSession() as session:
            data = await utils.fetch_url(
                session, COINGECKO_COIN_DATA_URL.replace("COIN_ID_REPLACE", coin_id)
            )
        ...
        async with ClientSession() as session:
            ohlc_data = await utils.fetch_url(
                session, COINGECKO_OHLC_DATA_URL.replace("COIN_ID_REPLACE", coin_id)
            )
        ...
        my_embed.set_author(
            name=f"{coin_name}({coin_symbol.upper()})", icon_url=coin_image
        )
        my_embed.add_field(name="Price", value=f"`{coin_price:,}$`", inline=True)
        my_embed.add_field(
            name="Market cap",
            value=f"`#{coin_market_cap_rank}\n{coin_market_cap:,}$`",
            inline=True,
        )
        my_embed.add_field(
            name="ATH", value=f"`{coin_ath:,}\n{coin_ath_real_date}`", inline=True
        )
        my_embed.add_field(
            name="Change(24h $)", value=f"`{coin_price_change_24h}$`", inline=True
        )
        my_embed.add_field(
            name="Change(24h %)",
            value=f"`{coin_price_change_percentage_24h}%`",
            inline=True,
        )
        my_embed.set_footer(text=f"Source: coingecko.com")

        fig = Figure(
            data=[Candlestick(x=dates, open=open, high=high, low=low, close=close)]
        )
